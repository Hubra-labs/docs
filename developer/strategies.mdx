...
| Stake actions | _unset_ — listing only |
| Unstake actions | _unset_ — listing only |
...
There are no `coming_soon` strategies live in the registry today. `sol-leveraged-stake` is `live` but listing-only — the registry entry is real but the stake / unstake builders aren't wired through the agent endpoints yet (see the section above).
