...
For `sol-leveraged-stake`, both `actions.stake` and `actions.unstake` are `null` — the registry entry is live and the `live` block carries real numbers, but the programmatic deposit / burn builders aren't agent-callable yet. See [Strategies → `sol-leveraged-stake`](/developer/strategies#sol-leveraged-stake) for the routing guidance.
